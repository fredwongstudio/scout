...
var AssistantCloudThreadHistoryAdapter = class {
	...
	withFormat(formatAdapter) {
		...
		const getFormatted = () => createFormattedPersistence(adapter._persistence, formatAdapter);
	...
	_reportRunTelemetry(format, runMessages, options, threadListItem) {
		...
		const remoteId = (threadListItem ?? this.aui.threadListItem).getState().remoteId;
		if (!remoteId) return;
