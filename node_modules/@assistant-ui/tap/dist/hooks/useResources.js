...
import { commitResourceFiber, discardWipRender, renderResourceFiber, unmountResourceFiber } from "../core/ResourceFiber.js";
...
function useResources(elements) {
	...
	const val = useRenderMemo(() => {
		...
		for (let i = 0; i < elements.length; i++) {
			...
			if (!state) {
				...
				if (typeof state.next === "object") discardWipRender(state.fiber);
	...
	useEffect(() => {
		return () => {
			for (const key of fibers.keys()) unmountResourceFiber(fibers.get(key).fiber);
		};
	}, [fibers]);
	useEffect(() => {
		for (const [key, state] of fibers.entries()) {
			...
			if (next === "delete") {
				...
				if (!state.fiber.isNeverMounted && !state.fiber.isMounted) commitResourceFiber(state.fiber);
			} else {
				if (next.remount) {
					unmountResourceFiber(state.fiber);
