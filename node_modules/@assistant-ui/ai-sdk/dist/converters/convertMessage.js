...
import { getToolName, isCustomContentUIPart, isReasoningFileUIPart, isToolUIPart } from "ai";
import { isMcpAppUri } from "@assistant-ui/core";
import { stableStringifyToolArgs } from "@assistant-ui/core/internal";
import { parsePartialJsonObject } from "assistant-stream/utils";
//#region src/converters/convertMessage.ts
...
function extractMcpAppMetadata(part, cache) {
	...
	if (app && typeof app === "object") a = app;
		...
		const ui = outMeta && typeof outMeta === "object" ? outMeta["ui"] : void 0;
		if (ui && typeof ui === "object" && typeof ui["resourceUri"] === "string" && isMcpAppUri(ui["resourceUri"])) {
			const uiMeta = ui;
			a = {
				resourceUri: uiMeta["resourceUri"],
				...Array.isArray(uiMeta["visibility"]) ? { visibility: uiMeta["visibility"] } : {}
			};
		} else {
			const flat = outMeta && typeof outMeta === "object" ? outMeta["ui/resourceUri"] : void 0;
			if (typeof flat !== "string" || !isMcpAppUri(flat)) return void 0;
			a = { resourceUri: flat };
		}
	}
	...
	if (typeof a["serverId"] === "string" && a["serverId"].length > 0) out.serverId = a["serverId"];
	if (cache) {
...
const uiPartStateToStatus = (state) => {
	...
	const converted = message.parts.filter((p) => p.type !== "step-start" && (message.role !== "user" || p.type !== "file")).map((part) => {
		...
		if (isToolUIPart(part)) {
			...
			return {
				...
				...part.callProviderMetadata != null ? { providerMetadata: part.callProviderMetadata } : void 0,
		...
		if (isReasoningFileUIPart(part)) return {
			type: "file",
			data: part.url,
			mimeType: part.mediaType
		};
		if (isCustomContentUIPart(part)) return {
			type: "data",
			name: part.kind,
