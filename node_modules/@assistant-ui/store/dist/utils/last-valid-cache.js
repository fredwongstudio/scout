//#region src/utils/last-valid-cache.ts
/**
...
*/
const createLastValidCache = (reportStale, scheduleExpiry) => {
	let last;
	let generation = 0;
	const expire = () => {
	...
	return { resolve: (valid, resolveItem) => {
		if (valid) {
			generation++;
			last = resolveItem();
			return last;
...
* throw may not be.
*/
const createStaleReporter = (options) => () => {
	if (!options.isCurrent()) return;
	try {
		if (options.isValid()) return;
	} catch {}
	console.error(`${options.name}: index ${options.index} is still out of bounds after the update settled; the scope throws on its next resolution.`);
};
//#endregion
export { createLastValidCache, createStaleReporter };

//# sourceMappingURL=last-valid-cache.js.map