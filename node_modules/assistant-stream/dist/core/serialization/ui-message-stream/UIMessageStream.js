...
//#region src/core/serialization/ui-message-stream/UIMessageStream.ts
...
var UIMessageStreamDecoder = class extends PipeableTransformStream {
	constructor(options = {}) {
		super((readable) => {
			...
			const emitPendingTool = (out, tool) => {
				if (!tool.emitted) {
					tool.emitted = true;
					out.enqueue({
					...
					for (const argsText of tool.deltas) out.enqueue({
				...
				transform(chunk, controller) {
					...
					if (isDataChunk(chunk)) {
						...
						if (options.onData) options.onData({
							...
							name,
							data: chunk.data,
							...chunk.transient !== void 0 && { transient: chunk.transient }
						});
						if (!chunk.transient) controller.enqueue({
							type: "data",
							path: [],
							data: [{
								name,
					...
					switch (type) {
						...
						case "tool-call-start": {
							...
							if (toolCallControllers.has(chunk.toolCallId)) throw new Error(`Encountered duplicate tool call id: ${chunk.toolCallId}`);
						...
						case "tool-result": {
							...
							if (!toolCallController) throw new Error(`Encountered tool result with unknown id: ${chunk.toolCallId}`);
							if (toolCallController.argsText === activeToolCallArgsText) activeToolCallArgsText = void 0;
			...
			return readable.pipeThrough(new TextDecoderStream()).pipeThrough(new SSEEventDecoderStream()).pipeThrough(new TransformStream({
				transform(event, controller) {
					if (event.event !== "message") throw new Error(`Unknown SSE event type: ${event.event}`);
					if (event.data === "[DONE]") {
						for (const tool of pendingTools.values()) emitPendingTool(controller, tool);
						receivedDone = true;
					...
					if (typeof chunk !== "object" || chunk === null || Array.isArray(chunk) || typeof chunk.type !== "string") {
						console.warn(`Dropped invalid UIMessageStream chunk: ${event.data.slice(0, 200)}`);
						return;
					}
					if (chunk.type === "text-delta" && chunk.textDelta === void 0) {
					...
					if (chunk.type === "tool-input-available" || chunk.type === "tool-input-error") {
						...
						if (chunk.input !== void 0) tool.deltas = [typeof chunk.input === "string" ? chunk.input : JSON.stringify(chunk.input)];
						if (chunk.type === "tool-input-error") tool.pendingResult = {
					...
					if (chunk.type === "tool-output-available" || chunk.type === "tool-output-error") {
						...
						tool.pendingResult = chunk.type === "tool-output-error" ? {
							result: typeof chunk.errorText === "string" ? chunk.errorText : "",
							isError: true
						} : {
							result: chunk.output,
