...
export const Thread = ({ components = EMPTY_COMPONENTS, className = "" }) => {
  const isEmpty = useAuiState(isNewChatView);

  return (
    <ThreadComponentsContext.Provider value={components}>
      <ThreadRoot isEmpty={isEmpty} className={className} />
    </ThreadComponentsContext.Provider>
  );
};

const ThreadRoot = ({ isEmpty, className = "" }) => {
  const {
  ...
  return (
    <ComposerPrimitive.Root className="aui-composer-root relative flex w-full flex-col">
      <ComposerPrimitive.AttachmentDropzone
        ...
        autoFocus
        enterKeyHint="send"
        aria-label="Message input" /><ComposerAction /></ComposerPrimitive.AttachmentDropzone>
...
const isRenderableAssistantPart = (part) => {
  switch (part.type) {
    case "text":
...
const AssistantMessage = () => {
  ...
  const showThinking = useAuiState(
    (state) => state.thread.isRunning && !state.message.parts.some(isRenderableAssistantPart)
  );
  ...
  return (
    <MessagePrimitive.Root
      ...
      <div
        ...
        {showThinking && ThinkingComponent ? <ThinkingComponent /> : (
